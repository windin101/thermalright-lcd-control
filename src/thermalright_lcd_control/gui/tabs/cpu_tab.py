...
"""
Tab widget for CPU metrics and graphs
"""
...
class CPUTab(QWidget):
    """Tab widget for CPU metrics configuration"""
    ...
    def setup_ui(self):
        ...
        # CPU Circular Graphs section
        arc_group = QGroupBox("CPU Circular Graphs")
        arc_layout = QVBoxLayout(arc_group)
        arc_layout.setContentsMargins(8, 12, 8, 8)
        arc_layout.setSpacing(6)
        
        for i in range(1, 3):  # 2 CPU circular graphs
            arc_name = f"cpu_arc{i}"
    ...
    def _create_bar_graph_row(self, bar_name, index):
        ...
        # Metric selector - CPU metrics only
        metric_label = QLabel("Metric:")
        row.addWidget(metric_label, alignment=Qt.AlignVCenter)
        metric_combo = QComboBox()
        metric_combo.addItem("CPU Usage", "cpu_usage")
        metric_combo.addItem("CPU Temp", "cpu_temperature")
        ...
        fill_btn.setStyleSheet("background-color: #55AAFF; border: 1px solid #bdc3c7; border-radius: 4px;")
    ...
    def _create_bar_gradient_row(self, bar_name):
        ...
        row = QHBoxLayout()
        row.setSpacing(6)
        row.setContentsMargins(55, 0, 0, 0)  # Indent
        
        # Container widget to show/hide
        container = QWidget()
